...
import { getAllSnapshots, deleteSnapshot, restoreSnapshot, addOperationLog, onDataChanged, clearAllLogs } from '@/services/storageService';
...
export const HistoryPage = () => {
  ...
  // 订阅数据变更：其他页面（批量整理/清理大师）写入日志或快照后自动刷新
  useEffect(() => {
  ...
  const handleRollback = async () => {
    ...
    try {
      // 执行真正的回滚（回滚前自动备份使用当前语言命名）
      await restoreSnapshot(selectedSnapshot.id, t('history.autoBackup'));
  ...
  const handleClearLogs = async () => {
    ...
    try {
      ...
      toast.success(t('history.clearLogsSuccess'));
    } catch (e) {
