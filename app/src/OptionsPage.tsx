...
import { estimateOrganizeTokens, DEFAULT_PROMPTS } from '@/services/aiService';
...
export default function OptionsPage() {
  ...
  // 当前激活的栏目（写入 URL hash，刷新页面后恢复）
  const VALID_TABS = ['ai', 'batch', 'clean', 'history', 'stats', 'about'];
  const [activeTab, setActiveTab] = useState(() => {
    const hash = window.location.hash.replace('#', '');
    return VALID_TABS.includes(hash) ? hash : 'ai';
  });
  
  // 切换栏目时同步到 URL hash（replaceState 不产生历史记录）
  ...
  useEffect(() => {
    ...
    // 根据显式 aiType 字段判断 AI 类型（兼容旧数据按 baseUrl 推断）
    if (settings.aiType) {
      setAiType(settings.aiType);
  ...
  const tokenEstimate = allBookmarks.length > 0 
    ? estimateOrganizeTokens(allBookmarks.slice(0, 50), allFolders.map(f => f.title), settings)
    : null;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
          ...
          {/* AI 设置（forceMount：切换 Tab 不卸载组件，批量整理等长任务不中断） */}
          <TabsContent value="ai" className="space-y-6" forceMount>
            <Card>
              <CardHeader>
              ...
              <CardContent className="space-y-6">
                ...
                <div className={`space-y-6 ${aiType === 'cloud' ? 'opacity-50 pointer-events-none' : ''}`}>
                  ...
                  <div className="flex items-start justify-between gap-4 rounded-lg border p-3">
                    <div className="space-y-1">
                      <Label htmlFor="ollamaThink" className="cursor-pointer">
                        {t('ai.thinkMode')}
                      </Label>
                      <p className="text-xs text-muted-foreground">
                        {t('ai.thinkModeHint')}
                    ...
                    <Switch
                      id="ollamaThink"
                      checked={formData.ollamaThink === true}
                      onCheckedChange={(checked) => setFormData({ ...formData, ollamaThink: checked })}
                      disabled={aiType === 'cloud'}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {([
                    { key: 'smartSave', labelKey: 'prompt.smartSave', varsKey: 'prompt.smartSaveVars' },
                    { key: 'organize', labelKey: 'prompt.organize', varsKey: 'prompt.organizeVars' },
                    { key: 'summary', labelKey: 'prompt.summary', varsKey: 'prompt.summaryVars' }
                  ] as const).map(({ key, labelKey, varsKey }) => {
                    const customized = !!formData.customPrompts?.[key];
                    return (
                      <details key={key} className="border rounded-lg p-3">
                        ...
                        <div className="mt-3 space-y-2">
                          <p className="text-xs text-muted-foreground break-all">
                            <span className="font-medium">{t('prompt.varsTitle')}: </span>
                            {t(varsKey)}
                          </p>
                          <Textarea
                            value={formData.customPrompts?.[key] ?? DEFAULT_PROMPTS[key]}
                          ...
                          {customized && (
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                const next = { ...formData.customPrompts };
                                delete next[key];
                                setFormData({ ...formData, customPrompts: next });
                              }}
                            >
                              {t('prompt.reset')}
                            </Button>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t('tags.vocabHint', { lang: formData.languagePreference === 'zh' ? CONFIG.LANGUAGES.zh : CONFIG.LANGUAGES.en })}
                  </p>
                  <Textarea
                    value={(formData.customControlledTags?.[formData.languagePreference] ?? CONFIG.CONTROLLED_TAGS[formData.languagePreference]).join(' ')}
                    onChange={(e) => {
                      const tags = e.target.value.split(/\s+/).filter(Boolean);
                      setFormData({
                        ...formData,
                        customControlledTags: {
                          ...formData.customControlledTags,
                          [formData.languagePreference]: tags
                        }
                      });
                    }}
                    rows={3}
                    ...
                    spellCheck={false}
                  />
                  {formData.customControlledTags?.[formData.languagePreference] && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        const next = { ...formData.customControlledTags };
                        delete next[formData.languagePreference];
                        setFormData({ ...formData, customControlledTags: next });
                      }}
                    >
                      {t('prompt.reset')}
                    </Button>
          ...
          {/* 批量整理 */}
          <TabsContent value="batch" className="space-y-6" forceMount>
            <BatchOrganize />
          </TabsContent>

          {/* 清理大师 */}
          <TabsContent value="clean" className="space-y-6" forceMount>
            <CleanMaster />
          </TabsContent>
          ...
          {/* 关于 */}
          <TabsContent value="about" className="space-y-6" forceMount>
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
